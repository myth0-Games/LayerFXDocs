...
1. Firstly, we should switch the background sprites to a sepearate Layer Mask. Select the parent game object, click on the layer dropdown, and change it to the desired layer mask. 
...
2. Also change your GlobalLight2D to a seperate layer, so we can capture it in the render pass. Here, we made another new layer called 'Lighting'. 

3. You will also want to deselect your desired layers from the Main Camera's culling mask. This prevents the desired layers from rendering twice (both on the Main Camera and from the Render Feature). 

4. Here, we deselected the 'Background' mask, but kept the 'Lighting' mask enabled because we want the lights to affect both the blurred layers and the main layers.
...
- **Tex:** Here, you can select a texture to render from as an alternative to render from a Layer Mask. This is useful for rendering layers with post processing effects. Leave this field
...
Forgetting to select the lighting layers will cause it to render only black!
